...
if(isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $sql = "SELECT * FROM student_coordinator s 
            INNER JOIN events e ON e.event_id = s.event_id 
            WHERE e.event_title LIKE '%$search%'
            LIMIT $start_from, $limit";
    $result = mysqli_query($conn, $sql);
    $total_sql = "SELECT COUNT(*) FROM student_coordinator s 
                  INNER JOIN events e ON e.event_id = s.event_id 
                  WHERE e.event_title LIKE '%$search%'";
} else {
    $sql = "SELECT * FROM student_coordinator s 
            ...
            LIMIT $start_from, $limit";
    $result = mysqli_query($conn, $sql);
    $total_sql = "SELECT COUNT(*) FROM student_coordinator";
}
...
<head>
    ...
    <title>Student Coordinator (ADMIN)</title>
...
<body>
    ...
    <div class="content">
        <div class="container" id="studentCoordinatorDetails">
            <h1 style="color: #710000; text-shadow: 2px 2px red; user-select: none; font-size: 40px;"><strong>STUDENT CO-ORDINATOR DETAILS <hr></strong></h1>
                    ...
                    <tbody>
                        <?php while($row = mysqli_fetch_array($result)) { ?>
                            <tr>
                                ...
                                <td><a href="updateStudent.php?id=<?php echo $row['event_id']; ?>" class="btn btn-success">Update</a></td>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const studentCoordinatorDetailsSection = document.getElementById('studentCoordinatorDetails');
            if (studentCoordinatorDetailsSection) {
                setTimeout(function() {
                    studentCoordinatorDetailsSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
                }, 100); 
            }
        });
    </script>
